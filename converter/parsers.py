...
"""
    ...
    def get_key_value_pairs(self, params):
        ...
        regex = r'([\w\.\d]+)[=\s+]((?:[{no_double_quotes}]+)|(?:\"[{no_double_quotes}]+\"))'.format(
            no_double_quotes=no_double_quotes)
    ...
    def ADD(self, params):
        ...
        print("Downloading `{dest}` from `{docker_image_name}`".format(dest=dest, docker_image_name=shlex.quote(
            self.docker_image_name)))
        # Make sure the image is pulled
        subprocess.run(
            "docker pull {docker_image_name}".format(docker_image_name=shlex.quote(self.docker_image_name)).split())
        ...
        try:
            ...
            files = [(s,
                      subprocess.run(cmd.format(dest=s, docker_image_name=shlex.quote(self.docker_image_name)).split(),
                                     stdout=subprocess.PIPE)) for s in names]
